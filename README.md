...
The easy smoke path is the runner image, which exercises the same app bring-up contract without running the AI user or producing a hosted verdict. Use it to verify, not guess, that the sandbox env is right before the first hosted PR run. It executes your repo's setup/seed/run or compose code, so the setup agent should strongly recommend it but wait for explicit approval before running it. For unfamiliar code, run it in a disposable dev VM or CI job.
